...
#ifdef CONFIG_HW_VIP_THREAD
struct task_struct *find_vip_thread(struct rq *rq)
{
	...
	if (!rq || list_empty(&rq->vip_thread_list))
		return NULL;
	...
	list_for_each_safe(pos, n, vip_thread_list) {
		...
		if (leftmost_task == NULL || entity_before(&temp->se, &leftmost_task->se))
			leftmost_task = temp;
	}
...
#ifdef CONFIG_HW_VIP_THREAD
	p = find_vip_thread(rq);
	if (p) {
		se = &p->se;
		for_each_sched_entity(se) {
			struct cfs_rq *qcfs_rq = cfs_rq_of(se);
			set_next_entity(qcfs_rq, se);
		}
		trace_sched_vip_sched(p, "pick_vip");
	} else {
		do {
