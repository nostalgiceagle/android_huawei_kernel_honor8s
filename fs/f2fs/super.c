...
repeat:
		...
		if (IS_ERR(page)) {
			...
			f2fs_msg(sb, KERN_ERR,
				...
				"(type=%d, err=%ld).", type, PTR_ERR(page));
...
retry:
		...
		if (unlikely(err)) {
			...
			f2fs_msg(sb, KERN_ERR,
				"Failed to write quota file "
				"(type=%d, err=%d).", type, err);
			set_sbi_flag(sbi, SBI_QUOTA_NEED_REPAIR);
			break;
		}
...
int f2fs_quota_sync(struct super_block *sb, int type)
{
	...
	if (ret) {
		f2fs_msg(sb, KERN_ERR,
			"Failed to sync quota file "
			"(type=%d, err=%d)", type, ret);
		goto out;
	}
...
skip_recovery:
	...
	f2fs_msg(sbi->sb, KERN_ALERT, "F2fs successfully mounted with "
		"checkpoint version = %llx", cur_cp_version(F2FS_CKPT(sbi)));
